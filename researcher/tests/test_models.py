...
# --- the tool log -----------------------------------------------------------
#
# Every model call in the pipeline goes through `Budgeted` — the planner, the
# extractor, the verifier, the sufficiency check and the synthesizer — so this
# one wrapper is the only place that has to know how to log one.

class MeteredFakeChatModel(FakeChatModel):
    """A fake that reports usage the way a provider does, so the wrapper's
    token accounting has something real to read."""

    def __init__(self, spec: str, meter: TokenMeter, tokens: int = 25, **kwargs):
        super().__init__(spec, **kwargs)
        self._meter, self._tokens = meter, tokens

    def invoke(self, prompt, **kwargs):
        self._meter.on_llm_end(usage(self._tokens))
...
def test_a_model_call_logs_the_role_it_was_made_for(caplog):
    """"A model call took 4s" is unactionable; "the extractor took 4s" points
    at a config line."""
    read = tool_logged(caplog)

    Budgeted(FakeChatModel("openai:gpt-4.1"), TokenMeter(100),
             role="extractor").invoke("hello")

    assert "extractor" in read()


def test_a_model_call_logs_which_model_answered(caplog):
    read = tool_logged(caplog)

    Budgeted(FakeChatModel("openai:gpt-4.1"), TokenMeter(100),
             role="planner").invoke("hello")

    assert "openai:gpt-4.1" in read()


def test_a_structured_call_logs_the_schema_it_asked_for(caplog):
    read = tool_logged(caplog)

    Budgeted(FakeChatModel("m"), TokenMeter(100), role="extractor") \
        .with_structured_output(Budget).invoke("x")

    assert "Budget" in read()


def test_a_model_call_logs_what_it_spent(caplog):
    read = tool_logged(caplog)
    meter = TokenMeter(1000)

    Budgeted(MeteredFakeChatModel("m", meter, tokens=25), meter,
             role="planner").invoke("hello")

    assert "25 tokens" in read()


def test_a_model_call_logs_a_preview_of_what_came_back(caplog):
    read = tool_logged(caplog)

    Budgeted(FakeChatModel("m"), TokenMeter(100), role="synthesizer").invoke("hi")

    assert "a report [1]" in read()


def test_the_prompt_is_logged_at_debug_not_info(caplog):
    """`-v` answers "what did the tools return?". Prompts are `-vv`: an
    extraction prompt carries a whole page of source text."""
    read = tool_logged(caplog)

    Budgeted(FakeChatModel("m"), TokenMeter(100),
             role="planner").invoke("the-whole-prompt")

    levels = {r.levelno for r in caplog.records if "the-whole-prompt" in r.getMessage()}
    assert levels == {logging.DEBUG}
    assert read()


def test_a_call_refused_by_the_budget_is_logged_as_a_failure(caplog):
    """The run stops here, and a stop with no log line is the worst kind."""
    read = tool_logged(caplog)
    meter = TokenMeter(max_total_tokens=10)
    meter.on_llm_end(usage(11))

    with pytest.raises(BudgetExceeded):
        Budgeted(FakeChatModel("m"), meter, role="planner").invoke("anything")

    assert "BudgetExceeded" in read()


def test_an_async_model_call_is_logged_too(caplog):
    read = tool_logged(caplog)

    asyncio.run(Budgeted(FakeChatModel("m"), TokenMeter(100),
                         role="verifier").ainvoke("hello"))

    assert "verifier" in read()


def test_every_built_role_knows_its_own_name(caplog):
    """The role has to travel from `build_models`, or the log says "extractor"
    only where somebody remembered to pass it."""
    read = tool_logged(caplog)
    models = build_models(ModelConfig(), factory=recording_factory())

    for role in ROLES:
        getattr(models, role).invoke("hello")

    logged = read()
    assert all(role in logged for role in ROLES)


def test_the_schema_survives_the_structured_binding_of_a_built_model(caplog):
    read = tool_logged(caplog)
    models = build_models(ModelConfig(), factory=recording_factory())

    models.extractor.with_structured_output(Budget).invoke("x")

    logged = read()
    assert "extractor" in logged and "Budget" in logged
