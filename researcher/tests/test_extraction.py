...
def test_a_fetch_logs_the_url_and_how_much_text_it_got(caplog):
    caplog.set_level(logging.DEBUG, logger="researcher.tools")

    research(fetch=FakeFetcher())
...
def test_a_failed_fetch_is_logged_with_its_error(caplog):
    caplog.set_level(logging.DEBUG, logger="researcher.tools")
...
def test_the_search_trace_records_which_backend_answered():
    """The trace is the curated channel, so it earns one extra field, not a
    dump: which backend a source came from is what tells a thin round from a
    degraded one."""
    out = research(router=FakeRouter(a_hit()))

    (event,) = kinds(out, "search")
    assert event["backend"] == "test"


def test_a_search_that_found_nothing_records_no_backend():
    out = research(router=FakeRouter())
