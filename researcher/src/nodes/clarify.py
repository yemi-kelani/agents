...
from toollog import LOGGER_NAME, tool_call

logger = logging.getLogger(f"{LOGGER_NAME}.clarify")
...
def clarify_node(state: ResearchState, *, llm) -> dict:
    ...
    for _ in range(MAX_ROUNDS):
        ...
        for tc in ai.tool_calls:
            ...
            if impl is None:
                # A hallucinated tool name is answered, not raised: killing the
                # run over one bad call is worse than proceeding on the
                # assumption. Logged rather than swallowed, though — it is a
                # model failure, and one that leaves no evidence behind cannot
                # be told from a model that simply chose not to ask.
                logger.warning("model called unknown tool %r; known: %s",
                               tc["name"], ", ".join(by_name))
                content = (f"No tool named {tc['name']}. "
                           f"Available: {', '.join(by_name)}.")
            else:
                # `GraphBubbleUp` passes through unremarked: `ask_user` leaves
                # by raising every time it pauses, and a warning per
                # clarification would teach everyone to skip the line that
                # means a tool really broke.
                with tool_call(tc["name"], (GraphBubbleUp,), **tc["args"]) as call:
                    content = impl.invoke(tc["args"])
                    call.set("answered", payload=content)
