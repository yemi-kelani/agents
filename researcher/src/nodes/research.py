...
async def research_topic_node(
    ...
    *,
    ...
    # The backend rides along because a round that fell back to the secondary
    # reads identically to one the primary served, and only one of those
    # explains a thin set of sources.
    trace = [ev("search", topic_id=topic.id, query=topic.question,
                n_results=len(hits),
    ...
    for hit in hits:
        ...
        if source_id not in store:
            ...
            if hit.content:
                ...
                fetches += 1
                try:
                    with tool_call("fetch", url=hit.url) as call:
                        text = await fetch(hit.url)
                        call.set(f"{len(text)} chars", payload=text)
