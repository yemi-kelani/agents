"""The one way to fetch a prompt.

Prompt text lives in `.txt` files beside this module, never inline in a node —
so a prompt can be read, diffed and edited without opening the code that uses
it. Nodes call `load("name")` and format the result themselves.
"""
