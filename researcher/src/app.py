...
def make_graph(config=None):
    ...
    """
    # The server never sees `run.py`'s argv, so `RESEARCHER_LOG_LEVEL` is the
    # only switch the tool log has here. Called before the build because that is
    # where a misconfigured backend raises (04).
    configure()
