...
The entrypoint may equally be `async def` — nothing else changes.
...
```python
# Invoke — wait for the result synchronously
...
Async: declare `async def my_workflow(...)` and `return await slow_computation(some_input)`.
