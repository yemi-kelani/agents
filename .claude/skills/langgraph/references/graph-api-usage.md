...
Any chat model works — install the provider package, set its API key, and build the model with
`init_chat_model`. See the [chat model integration docs](https://docs.langchain.com/oss/python/integrations/chat/).

| Provider | Install | Model | Environment |
|---|---|---|---|
| OpenAI | `"langchain[openai]"` | `init_chat_model("gpt-5.5")` | `OPENAI_API_KEY` |
| Anthropic | `"langchain[anthropic]"` | `init_chat_model("claude-sonnet-4-6")` | `ANTHROPIC_API_KEY` |
| Azure | `"langchain[openai]"` | `init_chat_model("azure_openai:gpt-5.5", azure_deployment=...)` | `AZURE_OPENAI_API_KEY`, `AZURE_OPENAI_ENDPOINT`, `OPENAI_API_VERSION` |
| Google Gemini | `"langchain[google-genai]"` | `init_chat_model("google_genai:gemini-3.7-flash")` | `GOOGLE_API_KEY` |
| AWS Bedrock | `"langchain[aws]"` | `init_chat_model("us.anthropic.claude-sonnet-4-6", model_provider="bedrock_converse")` | AWS credentials |
| HuggingFace | `"langchain[huggingface]"` | `init_chat_model("microsoft/Phi-3-mini-4k-instruct", model_provider="huggingface")` | `HUGGINGFACEHUB_API_TOKEN` |
| OpenRouter | `"langchain-openrouter"` | `init_chat_model("auto", model_provider="openrouter")` | `OPENROUTER_API_KEY` |

Each provider also has a direct class (`ChatOpenAI`, `ChatAnthropic`, …) if you prefer it over
`init_chat_model`.
